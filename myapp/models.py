...
class JobPosting(models.Model):
    ...
    def calculate_match_score(self, job_seeker):
        ...
        if self.salary_min and self.salary_max and job_seeker.expected_salary:
            if job_seeker.expected_salary < self.salary_min:
                salary_score = max(0, (job_seeker.expected_salary / self.salary_min) * 100)
            elif job_seeker.expected_salary > self.salary_max:
                salary_score = max(0, (self.salary_max / job_seeker.expected_salary) * 100)
            else:
                salary_score = 100
        else:
...
class JobSeeker(models.Model):
    ...
    phone = models.CharField(max_length=20, blank=True)
    preferred_job_types = models.ManyToManyField(JobPosting, related_name='preferred_by_jobseekers', blank=True)
